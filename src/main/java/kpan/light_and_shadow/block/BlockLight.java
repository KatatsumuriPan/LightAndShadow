...
public class BlockLight extends BlockBase implements IHasMultiModels, IBlockUncollidable {

    public static final PropertyInteger LIGHT = PropertyInteger.create("block_light_level", 0, 15);
    ...
    public boolean isTouchingItem(ItemStack stack) {
        return stack.getItem() == ItemInit.LIGHT;
    }
    ...
    @Override
    public int getMetaFromState(IBlockState state) { return state.getValue(LIGHT); }
    @Override
    public IBlockState getStateFromMeta(int meta) { return getDefaultState().withProperty(LIGHT, meta); }
    ...
    private static class Client {
        ...
        public static void renderParticle(World world, BlockPos pos) {
            if (Minecraft.getMinecraft().playerController.getCurrentGameType() != GameType.CREATIVE)
                return;
            ParticleBlockConditional particle = new ParticleLight(world, pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5);
